...
from .utils import UtilityFunction, acq_max, ensure_rng
...
class WrappedMatern(Matern):
    ...
    def _transform(self, X):
        ...
        for indices in categorical_group_indices.values():
            init_index = indices[0]
            max_col = np.argmax(X[:, indices], axis=1) + init_index
            X[:, indices] = 0
...
class BayesianOptimization(Observable):
    ...
    def __init__(self, f, pbounds, random_state=None, verbose=2,
                 bounds_transformer=None, mode='Nearest', ptype=None):
        self._random_state = ensure_rng(random_state)

        # Data structure containing the function to be optimized, the bounds of
        # its domain, and a record of the evaluations we have done so far
        self._space = TargetSpace(f, pbounds, ptype, random_state)
        ...
        if mode == 'Nearest':
            ...
            self._gp = GaussianProcessRegressor(
                kernel=WrappedMatern(param_types=ptype,nu=2.5),
                alpha=1e-2,
                normalize_y=False,
                n_restarts_optimizer=5,
                random_state=self._random_state,
